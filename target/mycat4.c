#include <stdio.h>
#include <stdlib.h>
#include <windows.h>
#include <fileapi.h>  // 用于GetFileInformationByHandleEx

// 获取最优缓冲区大小
size_t io_blocksize(const char* filename) {
    SYSTEM_INFO sys_info;
    GetSystemInfo(&sys_info);
    size_t mem_granularity = sys_info.dwAllocationGranularity;  // 内存分配粒度

    // 获取文件系统块大小
    HANDLE hFile = CreateFileA(
        filename,
        GENERIC_READ,
        FILE_SHARE_READ,
        NULL,
        OPEN_EXISTING,
        FILE_ATTRIBUTE_NORMAL,
        NULL
    );

    size_t fs_block_size = 4096;  // 默认值
    if (hFile != INVALID_HANDLE_VALUE) {
        FILE_STORAGE_INFO fsInfo;
        if (GetFileInformationByHandleEx(
            hFile,
            FileStorageInfo,
            &fsInfo,
            sizeof(fsInfo))) {
            
            // 验证块大小是否合理(2的幂且不小于512字节)
            if (fsInfo.PhysicalBytesPerSectorForPerformance >= 512 && 
                (fsInfo.PhysicalBytesPerSectorForPerformance & (fsInfo.PhysicalBytesPerSectorForPerformance - 1)) == 0) {
                fs_block_size = fsInfo.PhysicalBytesPerSectorForPerformance;
            }
        }
        CloseHandle(hFile);
    }

    // 计算最优缓冲区大小(内存粒度和文件系统块大小的最小公倍数)
    size_t buffer_size = mem_granularity;
    if (fs_block_size > buffer_size) {
        size_t remainder = fs_block_size % buffer_size;
        if (remainder != 0) {
            buffer_size = (fs_block_size / buffer_size + 1) * buffer_size;
        } else {
            buffer_size = fs_block_size;
        }
    }

    // 限制最大缓冲区大小(4MB)以避免过度消耗内存
    const size_t max_buffer_size = 4 * 1024 * 1024;
    if (buffer_size > max_buffer_size) {
        buffer_size = max_buffer_size;
    }

    return buffer_size;
}

// 对齐内存分配函数(与任务3相同)
char* align_alloc(size_t size) {
    SYSTEM_INFO sys_info;
    GetSystemInfo(&sys_info);
    size_t alignment = sys_info.dwAllocationGranularity;
    
    size_t total_size = size + alignment + sizeof(void*);
    void* original_ptr = malloc(total_size);
    if (!original_ptr) return NULL;
    
    void* aligned_ptr = (void*)(((size_t)original_ptr + alignment + sizeof(void*)) & ~(alignment - 1));
    *((void**)((char*)aligned_ptr - sizeof(void*))) = original_ptr;
    
    return (char*)aligned_ptr;
}

// 对齐内存释放函数(与任务3相同)
void align_free(void* ptr) {
    if (!ptr) return;
    void* original_ptr = *((void**)((char*)ptr - sizeof(void*)));
    free(original_ptr);
}

int main(int argc, char *argv[]) {
    if (argc != 2) {
        fprintf(stderr, "Usage: %s <filename>\n", argv[0]);
        return 1;
    }

    // 打开文件
    HANDLE hFile = CreateFileA(
        argv[1],
        GENERIC_READ,
        FILE_SHARE_READ,
        NULL,
        OPEN_EXISTING,
        FILE_ATTRIBUTE_NORMAL,
        NULL
    );

    if (hFile == INVALID_HANDLE_VALUE) {
        fprintf(stderr, "Error opening file: %lu\n", GetLastError());
        return 1;
    }

    // 获取最优缓冲区大小
    size_t buf_size = io_blocksize(argv[1]);
    printf("Using optimized buffer size: %zu bytes\n", buf_size);

    // 分配对齐缓冲区
    char *buf = align_alloc(buf_size);
    if (!buf) {
        fprintf(stderr, "Memory allocation failed\n");
        CloseHandle(hFile);
        return 1;
    }

    DWORD bytesRead;
    BOOL result;
    
    // 循环读取并输出文件内容
    while ((result = ReadFile(hFile, buf, buf_size, &bytesRead, NULL)) && bytesRead > 0) {
        if (fwrite(buf, 1, bytesRead, stdout) != bytesRead) {
            fprintf(stderr, "Error writing to stdout\n");
            align_free(buf);
            CloseHandle(hFile);
            return 1;
        }
    }

    align_free(buf);
    if (!CloseHandle(hFile)) {
        fprintf(stderr, "Error closing file: %lu\n", GetLastError());
        return 1;
    }

    return 0;
}